...
async function seedConfig() {
  try {
    ...
    if (!process.env.ADMIN_KEYPAIR) {
      ...
      console.error('   Generate one for yourself by calling: ./scripts/generate-admin-key.sh');
      process.exit(1);
    }
    ...
    try {
      ...
      console.log(`✅ Derived admin public key from ADMIN_KEYPAIR: ${adminPublicKey}`);
    } catch (error) {
      console.error('❌ Failed to parse ADMIN_KEYPAIR. Ensure it is a JSON array of numbers.');
      process.exit(1);
    }
