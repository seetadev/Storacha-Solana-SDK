...
/**
 * Updates the on-chain rate to match database config
 */
export async function updateOnChainRate(): Promise<void> {
  const { idl, programId } = await getIdlAndProgramId();
  const adminKeypair = await loadAdminKeypair();

  // set on-chain rate to 0 to disable validation
  // server already controls pricing dynamically based on USD rate and current SOL price
  const rateInLamports = 0;
  ...
  const updateIx = await program.methods
    .updateRate(new BN(rateInLamports))
    .accounts({
      config: configPda,
  ...
  try {
    ...
    await connection.confirmTransaction(sig, "confirmed");
    console.log(
      `✅ On-chain rate updated to ${rateInLamports} lamports. Tx: ${sig}`,
    );
  } catch (err) {
    console.error("Failed to update on-chain rate:", err);
    throw err;
  }
}
